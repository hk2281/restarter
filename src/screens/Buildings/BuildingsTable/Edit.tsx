...
import { CarOutlined, DeleteOutlined, HomeOutlined, IdcardOutlined, UploadOutlined } from "@ant-design/icons";
...
const BuildingEditModal: React.FC<BuildingModalProps> = ({ visible, onClose, data, onSave }) => {
    ...
    useEffect(() => {
        const fetchdata = async () => {
            // const userRsp: AxiosResponse = await api.get(`/buildings/${12}/users/`);
            try {
                // Получаем пользователей для target (по id здания)
                const targetResponse = await api.get(`/buildings/${data?.id}/users/`);
                ...
                // Фильтруем source, исключая тех, кто уже есть в target
                const filteredSourceData = sourceData.filter(
                    (user:any) => !targetData.some((targetUser:any) => targetUser.id === user.id)
                );

                // Добавляем данные в mockData
                setMockData([
                    ...filteredSourceData.map((user: any) => ({
                        key: user.id.toString(),
                        title: user.email, // Можешь изменить это на любое другое поле, которое хочешь показывать
                    })),
                    ...targetData.map((user: any) => ({
                        key: user.id.toString(),
                        title: user.email, // Аналогично, изменяй по желанию
                    })),
                ]);

                // Заполняем targetKeys из данных о пользователях в здании
                setTargetKeys(targetData.map((user: any) => user.id.toString()));
    ...
    const handleFinish = async(values: Backend.AdminBuilding) => {
        ...
        if(!deepEqual(
            ...
            const resp = await api.patch(`/buildings/${data?.id}`,transformToAdminBuilding(values))
            console.log('сохранены', resp.data)
        }else{
    ...
    const handleDeleteImage = async () => {
        try {
          ...
          setImageUrl(`http://localhost:8000/media`)
        ...
        >
            <Row gutter={[16, 16]}>
                <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                    ...
                    <BuildingPartsTable data={data?.building_parts || []} buildingId={data?.id} onUpdate={()=>{}}></BuildingPartsTable>
                </Col>
                <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                    <Row gutter={[16,16]}>
                        <Col span={24}>
                                    ...
                                    footer={(props) => {
                                        if (props.direction === 'right') {
                                            return (
                                                ...
                                                <Button onClick={handleRefreshSource} type="primary" >
                                                    Обновить
                                                </Button>
                                                </div>
                        ...
                        <Col xs={24} sm={24} md={24} lg={12} xl={12}>
                            <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                                ...
                                <Button icon={<DeleteOutlined />} danger style={{ width: "100%", marginBottom: 8 }} onClick={handleDeleteImage}>
                                    Удалить изображение
                                </Button>
